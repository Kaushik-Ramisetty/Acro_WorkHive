 ...
 *   • normalizeCandidate() maps backend shape → UI shape (_status, docs_uploaded, etc.)
...
const CANDIDATE_STATUSES = {
  ...
  _IN_PROGRESS: { label: ' In Progress',   color: 'bg-orange-100 text-orange-700'  },
  _CLEAR:       { label: ' Clear',         color: 'bg-green-100 text-green-700'    },
  _FAILED:      { label: ' Failed',        color: 'bg-red-100 text-red-700'        },
  _ON_HOLD:     { label: ' On Hold',       color: 'bg-orange-100 text-orange-700'  },
...
const _STATUS_CONFIG = {
  ...
  CLEAR:        { label: ' Cleared', color: 'bg-green-100 text-green-700',   icon: '✓' },
  ON_HOLD:      { label: ' On Hold', color: 'bg-orange-100 text-orange-700', icon: '⏸' },
...
// Mandatory document types that must be VERIFIED before  can start.
// Must match REQUIRED_DOC_TYPES in candidate_portal.py and _service.py.
const _REQUIRED_DOC_TYPES = new Set([
  '_form', 'aadhar', 'pan', 'qualification', 'address_proof', 'cif',
]);
...
const TIMELINE_STEPS = [
  ...
  { key: '_IN_PROGRESS', label: ' In Progress'       },
  { key: '_CLEAR',       label: ' Cleared'           },
  { key: 'CONVERTED',       label: 'Converted to Employee' },
  { key: 'JOINED',          label: 'Joined'                },
];

//  branch outcomes — shown as inline status badges, not timeline steps
const _BRANCH_STATUSES = new Set(['_IN_PROGRESS', '_CLEAR', '_ON_HOLD', '_FAILED']);
...
const POST_OFFER_STATUSES = new Set([
  ...
  '_IN_PROGRESS', '_CLEAR', '_ON_HOLD', '_FAILED',
  'CONVERTED', 'JOINED', 'NOT_JOINED',
]);

const TERMINAL_FAIL_STATUSES = new Set(['OFFER_REJECTED', '_FAILED', 'NOT_JOINED']);
const AVATAR_COLORS = [
...
function normalizeCandidate(raw) {
  ...
  // Backend now returns documents_verified and _initiable directly.
  ...
  const clientDocsVerified = [..._REQUIRED_DOC_TYPES].every((t) => verifiedDocTypes.has(t));
  ...
  return {
    ...
    // ──  (derived from nested _check) ──
    // Only show real  status & details once the offer is accepted.
    // For early pipeline statuses, always return NOT_STARTED and null _check
    // so stale DB records (wrong candidate_id associations) never show through.
    _status:   _earlyStatuses.has(raw.status)
                    ? 'NOT_STARTED'
                    : (raw._check?.status ?? 'NOT_STARTED'),
    _check:    _earlyStatuses.has(raw.status) ? null : (raw._check || null),
    // ── Documents (list of doc_type strings, deduped) ──
    docs_uploaded: [...new Set(rawDocs.map((d) => d.doc_type))],
    raw_documents: rawDocs,
    // ── Document verification gate (controls  Initiate button) ──
    // True only when ALL 6 required documents (_form, aadhar, pan,
    ...
    // _initiable: verified AND no _check record yet
    _initiable: allRequiredDocsVerified && !raw._check,
...
function getConfig(status) {
  return _STATUS_CONFIG[status] || _STATUS_CONFIG.NOT_STARTED;
}
...
function StatusTimeline({ currentStatus, Status }) {
  // Map  branch statuses onto the base timeline index so the
  // connector dots render correctly even for non-linear states.
  const effectiveStatus = (() => {
    switch (currentStatus) {
      case '_ON_HOLD': return '_IN_PROGRESS'; // on the _IN_PROGRESS step
      case '_FAILED':  return '_IN_PROGRESS'; // same position, different badge
      default:            return currentStatus;
    }
  })();

  const currentIdx = getStepIndex(effectiveStatus);
  const isFailed   = currentStatus === '_FAILED' || TERMINAL_FAIL_STATUSES.has(currentStatus);
  const isOnHold   = currentStatus === '_ON_HOLD';
  const isClear    = currentStatus === '_CLEAR';

  //  inline badge config
  const Badge = Status && Status !== 'NOT_STARTED' ? (
    _STATUS_CONFIG[Status] || { label: Status, color: 'bg-slate-100 text-slate-500', icon: '○' }
  ) : null;
  ...
  return (
    <div className="flex flex-col">
      {TIMELINE_STEPS.map((step, idx) => {
        ...
        // Special colour logic for  step
        const isStep = step.key === '_IN_PROGRESS' || step.key === '_CLEAR';
        const activeColor = isFailed && isStep ? 'bg-red-500 border-red-500'
                          : isOnHold && isStep ? 'bg-orange-500 border-orange-500'
                          : isClear  && isStep ? 'bg-emerald-500 border-emerald-500'
        ...
        return (
          <div key={step.key} className="flex items-start gap-3">
            ...
            <div className="flex flex-1 items-center gap-2">
              ...
              {/* Inline  badge next to the _IN_PROGRESS step */}
              {isStep && Badge && (active || done) && (
      ...
      {isFailed && currentStatus !== 'NOT_JOINED' && (
        <div className="mt-2 flex items-center gap-2 rounded-lg bg-red-50 px-3 py-2 text-sm text-red-600">
          <span>✗</span>
          <span> Failed — HR override is allowed</span>
        </div>
      )}
      {isOnHold && (
        <div className="mt-2 flex items-center gap-2 rounded-lg bg-orange-50 px-3 py-2 text-sm text-orange-700">
          <span>⏸</span>
          <span> On Hold — awaiting further review</span>
        </div>
      )}
      {TERMINAL_FAIL_STATUSES.has(currentStatus) && currentStatus !== '_FAILED' && (
        <div className="mt-2 flex items-center gap-2 rounded-lg bg-red-50 px-3 py-2 text-sm text-red-600">
          <span>⚠</span>
...
const DEPTS = ['Engineering', 'Product', 'Analytics', 'Marketing', 'Finance', 'HR', 'Operations'];

function CreateCandidateModal({ onClose, onCreated }) {
  const [form, setForm] = useState({
    first_name: '', last_name: '', email: '', phone: '',
    role: '', department: 'Engineering', ctc: '', joining_date: '',
  });
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="grid grid-cols-2 gap-4">
          {[
            ...
            { label: 'Role / Position *',key: 'role',        type: 'text',  placeholder: 'e.g. Backend Engineer' },
...
function OfferLetterModal({ candidate, onClose, onRefresh, toast }) {
  ...
  const hasOffer = !!(
    ...
    ['OFFER_GENERATED', 'OFFER_SENT', 'OFFER_ACCEPTED'
      ,
     'DOCS_PENDING', 'DOCS_SUBMITTED', '_IN_PROGRESS', '_CLEAR',
     '_FAILED', 'CONVERTED', 'JOINED'].includes(candidate.status)
  );
  ...
  useEffect(() => {
    getDepartments()
      .then((d) => {
        ...
        if (!candDept) {
          setMatchError('Candidate has no department on file. Please update the candidate profile before converting.');
          return;
        }
        ...
        if (dm) {
          ...
          setMatchError(`Department "${candidate.department}" is not configured. Please contact HR.`);
        }
      })
  ...
  useEffect(() => {
    ...
    getDesignations(form.department_id)
      .then((d) => {
        ...
        if (!candRole) {
          setMatchError((prev) => prev || 'Candidate has no role on file. Please update the candidate profile before converting.');
          return;
        }
        ...
        if (dg) {
          ...
          setMatchError((prev) => prev || `Designation "${candidate.role}" is not configured for the ${candidate.department} department. Please contact HR.`);
        }
      })
      ...
      <form onSubmit={handleSubmit}>
        <div className="max-h-[68vh] overflow-y-auto pr-2 space-y-7">
          ...
          <section>
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <FormField label="Email" hint="Locked — from candidate profile">
        ...
        <div className="mt-5 border-t border-slate-100 pt-4 flex items-center justify-between">
          <p className="text-xs text-slate-400">
            * Required · Employee code auto-generated · Password hashed with bcrypt
          </p>
 ...
 *
 * Step 1 of the  workflow: HR reviews each required document and marks it
 * VERIFIED or REJECTED.  Only after all 6 required documents are VERIFIED
 * is the "Initiate " button enabled (Step 2).
 *
 * Props:
    ...
    <Card title="Document Review (Step 1 — HR)">
      ...
      {allVerified ? (
        <div className="mb-4 flex items-center gap-2 rounded-lg border border-green-200 bg-green-50 px-4 py-3 text-sm text-green-700">
          ...
          <span>All required documents verified. HR can now initiate  below.</span>
        </div>
      ) : (
        <div className="mb-4 flex items-center gap-2 rounded-lg border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-700">
          <span className="text-base">⚠</span>
          <span>Verify all required documents before  can be initiated.</span>
        </div>
      )}

      <div className="divide-y divide-slate-100">
        {[..._REQUIRED_DOC_TYPES].map((docType) => {
          const doc       = docByType[docType];
          const label     = _DOC_LABELS[docType] || docType;
...
function CandidateDetail({ candidateId, onBack, toast }) {
  ...
  // ── Verify / Reject a document (HR Admin — Step 1 of  workflow) ─────────
  // Called by DocumentReviewPanel.  After each verify/reject we refresh the
  // candidate so the  button gate (all_required_docs_verified) updates.
  ...
  // ── Initiate  (HR Admin — Step 2, only after docs verified) ─────────────
  // Backend enforces:
  //   1. requires_role("admin","hr")  — candidates cannot call this.
  //   2. All required documents must be VERIFIED — returns 409 if not.
  //   3.  cannot be initiated twice — returns 409 if already started.
  const handleInitiate = async () => {
    setBusy(true);
    try {
      const result = await initiate(candidate.id);
      ...
      const expiryStr = expiry && !Number.isNaN(expiry.getTime())
        ...
        : 'Vendor link sent.';
      toast(`✅  initiated! ${expiryStr}`, 'success');
      await refreshCandidate();
    } catch (err) {
      toast(err.message || 'Could not initiate .', 'error');
    } finally {
      setBusy(false);
    }
  };
  ...
  const handleUpdate = async (newStatus) => {
    setBusy(true);
    try {
      const result = await updateStatus(candidate.id, newStatus);
      ...
      const cfg = _STATUS_CONFIG[newStatus];
  ...
  const canConvert    = ['OFFER_ACCEPTED','DOCS_PENDING','DOCS_SUBMITTED',
                         '_IN_PROGRESS','_CLEAR','_FAILED'].includes(status);
  ...
  const offerSent     = ['OFFER_SENT','OFFER_ACCEPTED','DOCS_PENDING','DOCS_SUBMITTED',
                          '_IN_PROGRESS','_CLEAR','_FAILED','CONVERTED','JOINED'].includes(status);
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <div className="mt-1.5 flex flex-wrap items-center gap-2">
              ...
              <Badge status={candidate._status} />
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        ...
        <div className="space-y-6 lg:col-span-1">
          ...
          <Card title="Onboarding Progress">
            <StatusTimeline currentStatus={status} Status={candidate._status} />
          </Card>
        </div>
        ...
        <div className="space-y-6 lg:col-span-2">
          ...
          {/*  */}
          <Card title="Background Verification ( — Step 2)">
            {/* Status row */}
            <div className="flex flex-wrap items-center justify-between gap-4 mb-4">
              <div>
                <p className="mb-2 text-sm text-slate-500">Current  Status</p>
                <Badge status={candidate._status} />
                {candidate._check?.vendor_name && (
                  <p className="mt-1 text-xs text-slate-400">Vendor: {candidate._check.vendor_name}</p>
                )}
                {candidate._check?.initiated_at && (
                  <p className="mt-0.5 text-xs text-slate-400">
                    Initiated: {new Date(candidate._check.initiated_at).toLocaleDateString()}
                  </p>
                )}
                {candidate._check?.completed_at && (
                  <p className="mt-0.5 text-xs text-slate-400">
                    Completed: {new Date(candidate._check.completed_at).toLocaleDateString()}
                  </p>
                )}
                {candidate._check?.remarks && (
                  <p className="mt-1 text-xs text-slate-500 italic">"{candidate._check.remarks}"</p>
                )}
              </div>

              {/* ──  Initiation Gate ──────────────────────────────────────────
                  RULE: HR Admin can initiate  ONLY after ALL required
                  ...
                  {candidate.all_required_docs_verified ? (
                    ...
                    <>
                      ...
                      <p className="text-xs text-amber-700 text-right max-w-xs">
                        ⚠ All required documents must be verified (Step 1) before  can be initiated.
                      </p>
                    </>
                  )}
                </div>
            ...
            {candidate._status !== 'NOT_STARTED' && (
              <div>
                <p className="mb-2 text-xs text-slate-400">Admin override — update  status manually</p>
                <div className="grid grid-cols-2 gap-2">
                  {Object.entries(_STATUS_CONFIG).filter(([k]) => k !== 'PENDING' && k !== 'NOT_STARTED').map(([key, cfg]) => {
                    const isActive = candidate._status === key;
                    return (
                      <button
                        key={key}
                        type="button"
            ...
            {status === '_FAILED' && (
              <div className="mt-4 rounded-lg border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-700">
                ⚠  failed. You can still convert the candidate — HR override is allowed.
              </div>
            )}
            {status === '_CLEAR' && (
              <div className="mt-4 rounded-lg border border-green-200 bg-green-50 px-4 py-3 text-sm text-green-700">
                ✅  cleared. Candidate is eligible for conversion to employee.
              </div>
            )}
          </Card>
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        subtitle="End-to-end workflow: offer → documents →  → employee activation"
        right={
        ...
        <div className="overflow-x-auto">
          <table className="w-full">
            ...
            <tbody>
              {loading ? (
                <tr>
                  <td colSpan={8} className="py-16 text-center text-sm text-slate-400 animate-pulse">
                ...
                filtered.map((c) => (
                  <tr key={c.id} className="border-b border-slate-50 transition hover:bg-blue-50/40">
                    ...
                    <td className="px-4 py-3"><DocumentStatusBadge docs={c.docs_uploaded} /></td>
                    <td className="px-4 py-3"><Badge status={c._status} /></td>
