...
def _run_onboarding_migrations() -> None:
    ...
    safe_alters = [
        ...
        # Recruitment → Onboarding bridge columns.
        "ALTER TABLE recruitment_candidates ADD COLUMN onboarded_at DATETIME",
        "ALTER TABLE recruitment_candidates ADD COLUMN onboarded_by INTEGER",
...
def _run_recruitment_migrations() -> None:
    """Idempotent ALTERs for the Recruitment module's schema additions."""
    from sqlalchemy import text

    safe_alters = [
        # InterviewRound — slot-selection token fields
        "ALTER TABLE interview_rounds ADD COLUMN selection_token VARCHAR(64)",
        "ALTER TABLE interview_rounds ADD COLUMN token_used     INTEGER DEFAULT 0",
        "ALTER TABLE interview_rounds ADD COLUMN email_sent_at  DATETIME",
        # InterviewSlot — denormalised lookup fields
        "ALTER TABLE interview_slots ADD COLUMN candidate_id   VARCHAR(20)",
        "ALTER TABLE interview_slots ADD COLUMN interviewer_id VARCHAR(20)",
        "ALTER TABLE interview_slots ADD COLUMN requirement_id INTEGER",
        # Recruiter assignment audit trail
        "ALTER TABLE job_requirements ADD COLUMN assigned_by INTEGER",
        "ALTER TABLE job_requirements ADD COLUMN assigned_at DATETIME",
    ]
    with engine.connect() as conn:
        for stmt in safe_alters:
            try:
...
_INITIAL_SKILLS = [
    ("SKL001", "React"),        ("SKL002", "Node.js"),      ("SKL003", "Python"),
    ("SKL004", "PostgreSQL"),   ("SKL005", "TypeScript"),   ("SKL006", "GraphQL"),
    ("SKL007", "Docker"),       ("SKL008", "AWS"),           ("SKL009", "Redux"),
    ("SKL010", "Kubernetes"),   ("SKL011", "TensorFlow"),   ("SKL012", "Selenium"),
    ("SKL013", "JavaScript"),   ("SKL014", "HTML"),          ("SKL015", "CSS"),
    ("SKL016", "RESTful API"),  ("SKL017", "MySQL"),         ("SKL018", "Java"),
    ("SKL019", "Angular"),      ("SKL020", "Dotnet"),
]


def _seed_skill_set() -> None:
    """Idempotent seed for skill_set master table."""
    from sqlalchemy.orm import Session as _OrmSession
    from app.models.skill_set import SkillSet

    with _OrmSession(engine) as session:
        for skill_id, skill_name in _INITIAL_SKILLS:
            if not session.get(SkillSet, skill_id):
                session.add(SkillSet(id=skill_id, skill_name=skill_name,
                                     is_active=1, added_by="system"))
        session.commit()
        logger.info("skill_set ready: %d skills", session.query(SkillSet).count())
...
def _seed_master_data() -> None:
    ...
    with Session(engine) as session:
        ...
        for row in _LEAVE_TYPES:
            if not session.get(LeaveType, row["id"]):
...
def _backfill_null_designations() -> None:
    """Assign a default designation to employees whose designation_id is NULL.

    Admin-role employees in the Management department (DEP020) with no
    designation get 'Leadership' (D81), which is the canonical designation for
    that combination.  All other employees with NULL designation_id are left
    untouched — their correct designation must be set via the admin UI.

    Idempotent: once designation_id is set, this routine never overwrites it.
    """
    ...
    try:
        with engine.begin() as conn:
            result = conn.execute(_text("""
                UPDATE employees
                SET designation_id = 'D81'
                WHERE designation_id IS NULL
                  AND is_deleted = 0
                  AND department_id = 'DEP020'
                  AND role_id = (SELECT id FROM roles WHERE name = 'admin' LIMIT 1)
            """))
            n = result.rowcount or 0
        if n:
            logger.info(
                "_backfill_null_designations: assigned 'Leadership' (D81) to %d "
                "admin/Management employee(s) with NULL designation_id",
                n,
            )
    except Exception:
        logger.exception("_backfill_null_designations failed (continuing).")
...
async def lifespan(app: FastAPI):
    ...
    # ── Readonly startup mode ─────────────────────────────────────────────────
    # APP_ENV=readonly starts the server with zero startup mutations.
    # Every function call below that can INSERT, UPDATE, DELETE, or COMMIT is
    # skipped. API routes and database reads remain fully available so the app
    # can be browsed and tested without touching hrms.db.
    # Development (APP_ENV=development) and production (APP_ENV=production)
    # behaviour is unchanged.
    if (settings.APP_ENV or "").lower() == "readonly":
        logger.info(
            "HRMS starting in READONLY mode — all startup mutations are skipped. "
            "API routes and database reads are fully available."
        )
...
from app.routes import interviewer as interviewer_routes
from app.routes import candidate_slot as candidate_slot_routes
from app.routes import interviewer_slot as interviewer_slot_routes
app.include_router(recruitment_routes.router)      # /recruitment
app.include_router(recruiter_routes.router)        # /recruiter
app.include_router(interviewer_routes.router)      # /interviewer
app.include_router(candidate_slot_routes.router)   # /candidate  (public, no auth)
app.include_router(interviewer_slot_routes.router) # /interviews (public, no auth)
