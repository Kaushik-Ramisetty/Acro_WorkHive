...
# ─────────────────────────────────────────────────────────────────────────────
# Skill-Set Master — HR Admin approval workflow
# ─────────────────────────────────────────────────────────────────────────────
...
@router.get("/skill-requests")
def list_skill_requests(
    status_filter: Optional[str] = Query(None, alias="status",
                                         description="pending | approved | rejected"),
    db: Session = Depends(get_db),
):
    """HR Admin — list all global skill requests."""
    rows = skill_svc.list_skill_requests(db, status=status_filter)
    result = []
    for r in rows:
        req_emp = db.query(Employee).filter(Employee.id == r.requested_by).first() if r.requested_by else None
        apr_emp = db.query(Employee).filter(Employee.id == r.approved_by).first()  if r.approved_by else None
        result.append({
            "id":               r.id,
            "skill_name":       r.skill_name,
            "status":           r.status,
            "requested_by_id":  r.requested_by,
            "requested_by":     req_emp.full_name if req_emp else None,
            "requested_at":     r.requested_at.isoformat(),
            "approved_by":      apr_emp.full_name if apr_emp else None,
...
@router.post("/skill-requests/{request_id}/approve")
def approve_skill_request(
    request_id: int,
    db: Session = Depends(get_db),
    current: Employee = Depends(role_required("admin")),
):
    """HR Admin approves a pending skill request — inserts skill into skill_set master table."""
    req = skill_svc.approve_skill_request(db, request_id=request_id, approved_by=current.id)
    if not req:
    ...
    return {
        ...
        "message":     f"'{req.skill_name}' approved and added to master skill list.",
    }


@router.post("/skill-requests/{request_id}/reject")
def reject_skill_request(
    ...
    req = skill_svc.reject_skill_request(
        db, request_id=request_id, approved_by=current.id, reason=reason
    )
    if not req:
        raise HTTPException(status_code=404, detail="Skill request not found.")
    return {
        ...
        "skill_name": req.skill_name,
        "status":     req.status,
        "message":    f"'{req.skill_name}' request rejected.",
    }


@router.get("/skill-set")
def list_skill_set(db: Session = Depends(get_db)):
    """HR Admin — view all skills in the master skill_set table."""
    skill_svc.ensure_skill_set_added_at(db)
    rows = db.query(SkillSet).order_by(SkillSet.skill_name).all()
    return [
        {"id": r.id, "skill_name": r.skill_name, "is_active": r.is_active,
