...
def sync_master_data(session, *, overwrite_department_id: bool = True) -> None:
    """Synchronize the master tables to the HTML mapping."""
    from app.models import Department, Designation

    official_departments = {row["id"] for row in OFFICIAL_DEPARTMENTS}
    official_designations = {row["id"] for row in OFFICIAL_DESIGNATIONS}

    for dept in list(session.query(Department).all()):
        if dept.id not in official_departments:
            session.delete(dept)
    session.flush()

    for row in OFFICIAL_DEPARTMENTS:
        dept = session.get(Department, row["id"])
        if dept is None:
            ...
            dept.name = row["name"]
            dept.parent_department_id = row["parent_department_id"]
    session.flush()

    for desig in list(session.query(Designation).all()):
        if desig.id not in official_designations:
    ...
    for row in OFFICIAL_DESIGNATIONS:
        ...
        desig = session.get(Designation, row["id"])
        if desig is None:
        ...
        desig.title = title
        desig.level = row["level"]
        if overwrite_department_id or desig.department_id != department_id:
