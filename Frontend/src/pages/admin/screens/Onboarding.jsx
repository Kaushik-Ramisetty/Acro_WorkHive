...
function ConvertToEmployeeModal({ candidate, onClose, onRefresh, toast }) {
  ...
  const [form, setForm] = useState({
    ...
    secondary_phone: '',
    // Employment — department_id and designation_id are ALWAYS chosen by
    // the admin at conversion time; never locked from candidate profile.
    department_id:       '',
    designation_id:      '',
  ...
  // ── Load departments — attempt to pre-select from candidate.department ────────
  // If the candidate already has a department string that matches a DB row
  // (e.g. legacy or direct-entry onboarding), pre-select it as a convenience.
  // The admin can always override — the dropdown is always editable.
  useEffect(() => {
    getDepartments()
      .then((d) => {
  ...
  // ── Load designations when department changes — attempt pre-select ────────────
  useEffect(() => {
    ...
    getDesignations(form.department_id)
      .then((d) => {
        ...
        // Try to pre-select from candidate.role string (best-effort, not blocking).
  ...
  // ── Validation ────────────────────────────────────────────────────────────────
  // Department and Designation are required at conversion time and must be
  // explicitly selected by the admin — they are never auto-locked from the
  // candidate's profile string.
  const validate = () => {
    const e = {};
    if (!form.first_name.trim())   e.first_name        = 'Required';
    if (!form.last_name.trim())    e.last_name         = 'Required';
    if (!form.department_id)       e.department_id     = 'Required — select a department';
    if (!form.designation_id)      e.designation_id    = 'Required — select a designation';
          ...
          <section>
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <FormField label="Department" required error={err('department_id')}
                hint="Select the department for this employee">
                <select
                  className={INPUT_CLS}
              ...
              <FormField label="Designation" required error={err('designation_id')}
                hint={form.department_id ? 'Select the designation for this employee' : 'Select a department first'}>
                <select
                  className={INPUT_CLS}
