...
import { PageTitle, Card } from "../parts/UI";
import { usePersistedState, useSaveFeedback } from "../../../hooks/usePersistedState";

// Tabs mirror the manager Settings page: Profile / Security / Notifications
// / Appearance. The legacy Language tab has been removed.
const TABS = [
...
const SettingsPage = () => {
  ...
  const initials = (fullName || 'U').split(' ').filter(Boolean).map((x)=>x[0]).slice(0,2).join('').toUpperCase() || 'U';
          ...
          <ul className="space-y-1">
            {TABS.map((t) => (
              <li key={t.id}>
                <button
                  onClick={() => setTab(t.id)}
                  className={`w-full flex items-center gap-2 px-3 py-2 rounded-lg text-xs font-semibold transition-colors ${tab === t.id ? "bg-teal-50 text-teal-600" : "text-slate-600 hover:bg-slate-50"}`}
                >
          ...
          {tab === "profile" && (
            <div>
              ...
              <div className="flex items-center gap-4 mb-5">
                <div className="w-16 h-16 rounded-2xl bg-gradient-to-br from-teal-400 to-blue-500 flex items-center justify-center text-white font-bold text-xl">{initials}</div>
                <div className="flex flex-col gap-2">
                  <button className="px-3 py-1.5 rounded-lg text-xs font-semibold text-white" style={{ background: "#14b8a6" }}>Upload Photo</button>
                  <button className="px-3 py-1.5 rounded-lg text-xs font-semibold text-slate-600 border border-slate-200">Remove</button>
                </div>
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {[
                  ...
                  <div key={l}>
                    <label className="block text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1">{l}</label>
                    <input defaultValue={v} className="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm text-slate-700 focus:outline-none focus:ring-2 focus:ring-teal-400 focus:border-transparent" />
                  </div>
                ))}
              </div>
              <ProfileSaveButton />
            ...
            <div>
              ...
              <div className="space-y-4 max-w-md">
                {["Current Password", "New Password", "Confirm New Password"].map((l) => (
                  <div key={l}>
                    <label className="block text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1">{l}</label>
                    <input type="password" placeholder="••••••••" className="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm text-slate-700 focus:outline-none focus:ring-2 focus:ring-teal-400 focus:border-transparent" />
                  </div>
                ))}
                <button className="px-4 py-2 rounded-lg text-xs font-semibold text-white" style={{ background: "#14b8a6" }}>Update Password</button>
              </div>
              <div className="mt-6 p-4 rounded-xl bg-slate-50 border border-slate-100">
                <p className="text-sm font-semibold text-slate-700">Two-Factor Authentication</p>
                <p className="text-xs text-slate-500 mt-1 mb-3">Add an extra layer of security to your account.</p>
                <button className="px-4 py-1.5 rounded-lg text-xs font-semibold border border-teal-300 text-teal-600 hover:bg-teal-50">Enable 2FA</button>
              </div>
            </div>
          )}
...
function NotifList() {
  const [state, setState] = usePersistedState(
    'hrms.settings.employee.notifs',
    NOTIFS.map(([, , on]) => on),
  );
  // Defensive: if the persisted shape ever drifts (e.g., we add a new
  // notification row later), normalise length here.
    ...
    <ul>
      {NOTIFS.map(([label, desc], i) => (
        <li key={i} className="flex items-center justify-between py-3 border-b border-slate-100 last:border-0">
          ...
          >
            <span className="absolute top-0.5 w-4 h-4 bg-white rounded-full shadow transition-all" style={{ left: flags[i] ? "calc(100% - 18px)" : "2px" }} />
          </button>
        </li>
      ))}
    </ul>
...
// Manager-style Light/Dark theme picker — two preview cards with a radio
// indicator. The teal accent matches the rest of the employee shell. The
// preference persists in localStorage; the actual app-wide dark-mode wiring
// isn't done yet, so this just stores the choice for now.
const THEME_OPTIONS = [
  {
    id: 'light',
    label: 'Light',
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5 max-w-2xl">
        {THEME_OPTIONS.map((opt) => {
          ...
          return (
            ...
            >
              {/* Mini preview of the theme — surface + a card row + an accent dot. */}
              <div
