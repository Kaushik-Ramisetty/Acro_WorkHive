...
export default function ManagerApprovalsPage() {
  // Live leave items waiting on this manager (server-scoped via pendingMyApproval).
  ...
  // Comp-off + regularization queues — fetched here so this page covers ALL
  // manager-scoped approvals, not just leave.
  ...
  const refreshAux = async () => {
    ...
    try {
      ...
      setCompOffs(Array.isArray(co) ? co.filter((c) => c.next_approver_role === 'manager') : []);
      setRegs(Array.isArray(rg) ? rg : []);
    } catch (e) {
      setAuxErr(e?.message || 'Failed to load comp-off / regularization queue');
    }
  };
  ...
  // Build a unified list with type-tagged items so we can render each kind
  // with the right approve/reject hookup.
  const items = useMemo(() => {
